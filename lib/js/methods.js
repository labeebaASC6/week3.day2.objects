...
heroCreator.createCharSheet = function(heroObj){
    ...
    heroContainer.setAttribute("class", "hero_container");

    const heroImg = document.createElement("IMG");
    heroImg.setAttribute("src", `./assets/${heroObj.img}`);
    heroContainer.appendChild(heroImg);

    const infoDiv = this.createInfoDiv(heroObj);   // iwannagiveupnamingthings
...
heroCreator.createInfoDiv = function(heroObj){
    const heroContainer = document.createElement("div");
    heroContainer.setAttribute("class", "hero_text_container");
    const ogodwhatsgoinon = Object.keys(heroObj);   //aka an array of hero props
    for(let i in ogodwhatsgoinon)
    {
        var heroInfo;
        if(i != "abilities")    //will not print the abilities array
        {
            heroInfo = document.createElement("p");
            heroInfo.innerText = heroObj[ogodwhatsgoinon[i]];
        }
        else
        {
            heroInfo = getAbilityString(ogodwhatsgoinon[i]);
        }
...
heroCreator.getAbilityString = function(abilitiesArr){
    var heroAbility = "";
    for(const i in abilitiesArr)
    {
        heroAbility += `${abilitiesArr["name"]}, `;
    }
